    ...
    private void cbKdMapelItemStateChanged(java.awt.event.ItemEvent evt) {//GEN-FIRST:event_cbKdMapelItemStateChanged
        try {
            if (!cbKdMapel.getModel().getSelectedItem().equals("-Pilihan-")) {
                String sql = "SELECT pelajaran.mata_pelajaran,guru.nama FROM pelajaran,detail_pelajaran,guru WHERE pelajaran.kode_pelajaran = detail_pelajaran.kode_pelajaran AND guru.nik = detail_pelajaran.nik AND detail_pelajaran.kode_detail_pelajaran ='"+cbKdMapel.getModel().getSelectedItem().toString()+"'";
                java.sql.Connection conn = (Connection) db_connection.configDB();
                Statement st = conn.createStatement();
                rs = st.executeQuery(sql);
                if (rs.next()) {
